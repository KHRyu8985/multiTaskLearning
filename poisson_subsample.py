...
class PoissonDiskMaskFunc(MaskFunc):
    def __call__(
        ...
        with temp_seed(self.rng, seed):
            ...
            num_lines = (num_cols / acceleration - num_low_freqs)
            num_lines = np.int(num_lines)
            poisson_generator = PoissonGenerator(num_dim=1, disk=False, repeatPattern=True, first_point_zero=False)
            points = poisson_generator.find_point_set(num_points=num_lines, num_iter=5, iterations_per_point=5, rotations=1)
            points = np.array(points * num_cols).astype('int')
            mask[points] = True
            # Added Center fractions
